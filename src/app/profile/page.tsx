...
export default function ProfilePage() {
  ...
  if (step === 2) {
    return (
      <div className="page-enter min-h-screen page-container pt-10">
      ...
      <div className="mb-6">
        <div className="flex gap-1.5 mb-4">
          {[0, 1].map((i) => (
            <div
              key={i}
              className={`flex-1 h-1 rounded-full ${i <= step ? "bg-blue-500" : "bg-gray-700"}`}
            />
          ))}
        </div>
        ...
        <div className="space-y-5 max-w-2xl">
          ...
          <div>
            <label className="text-xs text-gray-400 block mb-2">
              Budget range: {"$".repeat(profile.budgetRange)}
            </label>
            <input
              type="range"
              min={1}
              ...
              onChange={(e) =>
                setProfile((p) => ({ ...p, budgetRange: +e.target.value as 1 | 2 | 3 }))
              }
          ...
          <div>
            <label className="text-xs text-gray-400 block mb-2">
              Max travel: {profile.maxTravelMinutes} min
            </label>
            <input
              type="range"
              min={10}
              ...
              onChange={(e) =>
                setProfile((p) => ({ ...p, maxTravelMinutes: +e.target.value }))
              }
              className="w-full accent-blue-500"
            />
          </div>
          ...
          <div>
            <label className="text-xs text-gray-400 block mb-2">
              Party size: {profile.partySize ?? 2}
            </label>
            <input
              type="range"
              min={1}
              max={10}
              value={profile.partySize ?? 2}
              onChange={(e) =>
                setProfile((p) => ({ ...p, partySize: +e.target.value }))
              }
              className="w-full accent-blue-500"
            />
          </div>
            ...
            >
              <span
                className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-all ${
                  profile.notificationsEnabled ? "left-6" : "left-0.5"
                }`}
              />
            </button>
          </div>
          ...
          <div className="flex gap-2">
            <button
              onClick={() => setStep(0)}
              className="px-4 py-3 rounded-xl border border-gray-700 text-gray-400 hover:text-white transition-colors"
            >
              ← Back
            </button>
            <button
