...
function RouteContent() {
  ...
  const share = async () => {
    if (!venue || !match) return;
    const url = `${window.location.origin}/route?venueId=${venue.id}&matchId=${match.id}`;
    if (navigator.share) {
      await navigator.share({ title: `Route to ${venue.name}`, url });
    } else {
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center text-gray-400">
          ...
          <Link href="/venue" className="text-blue-400 underline">
            Browse venues →
          </Link>
        </div>
      </div>
      ...
      <div className="bg-gray-900 border-b border-gray-800">
        <div className="page-container pt-10 pb-4">
          ...
          <p className="text-sm text-gray-400">
            {venue.name} · {match.homeTeam.name} vs {match.awayTeam.name}
          </p>
        </div>
      </div>

      <div className="page-container pt-4 pb-4 space-y-4">
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
          <div
            ...
            style={{ height: "min(58vh, 520px)", minHeight: 280, background: "#111827" }}
          >
            <RouteMap venue={venue} mode={mode} />
          </div>
          ...
          <div className="space-y-4">
            {departureBrief && (
              <div className="bg-blue-950/50 border border-blue-800 rounded-2xl p-4">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="text-xs text-blue-400 font-semibold mb-1">Departure Brief</p>
            ...
            {plan && !loading && (
              <div className="bg-gray-800 rounded-2xl border border-gray-700 p-4">
                ...
                <div className="mt-3 pt-3 border-t border-gray-700 flex flex-wrap justify-between gap-2 text-xs text-gray-400">
                  <span>
                    Leave by{" "}
                    <strong className="text-white">
                      {new Date(plan.departureTime).toLocaleTimeString("en-US", {
                        hour: "numeric",
                        minute: "2-digit",
                  ...
                  <span>
                    Arrive by{" "}
                    <strong className="text-white">
                      {new Date(plan.arrivalTime).toLocaleTimeString("en-US", {
                        hour: "numeric",
                        minute: "2-digit",
                        timeZone: "America/Los_Angeles",
                      })}
                  ...
                  <span className="text-gray-500">~{travelMinutes} min total</span>
                </div>
              </div>
            )}
