...
import {
  ...
  Star, AlertTriangle, ChevronDown, Clock, Zap, ArrowDown, PhoneCall, Sparkles, Brain,
} from "lucide-react";
...
export default function DiscoverPage() {
  ...
  // Sort by AI rank score if available, otherwise by confidence
  const aiRankMap = Object.fromEntries(aiRankings.map((r) => [r.venueId, r]));
  const sortedFiltered = [...filtered].sort((a, b) => {
    const scoreA = aiRankMap[a.id]?.aiRankScore ?? a.confidence / 100;
    const scoreB = aiRankMap[b.id]?.aiRankScore ?? b.confidence / 100;
  ...
  // Fetch AI rankings whenever the match changes
  const fetchAiRankings = useCallback(
    async (venuesToRank: Venue[], match: Match) => {
      if (!venuesToRank.length) return;
      setAiRankLoading(true);
      setAiRankDone(false);
      setAiRankings([]);
      try {
        const res = await fetch("/api/rocketride/venue-rank", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            venues: venuesToRank,
            preferences,
            matchContext: `${match.homeTeam} vs ${match.awayTeam} at ${match.venueName}, ${match.venueCity}`,
          }),
        });
        if (res.ok) {
          const data = await res.json();
          if (Array.isArray(data.rankings)) setAiRankings(data.rankings);
        }
      } catch {
        // silently fail — fall back to static order
      } finally {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setLoading(false);
      void fetchAiRankings(venues, activeMatch);
    }, 650);
  ...
  return (
    <div className="page-enter pb-6">
      <div className="page-container pt-5 space-y-5">
              ...
              <motion.button
                ...
                style={{
                  ...
                  background: isSel
                    ...
                    : "rgba(10,18,8,0.65)",
                  border: isSel ? "1px solid rgba(204,255,0,0.35)" : "1px solid rgba(255,255,255,0.055)",
                  boxShadow: isSel ? "0 10px 28px rgba(0,0,0,0.45)" : "none",
                }}
              >
                {isSel && (
                  <div className="absolute top-0 left-0 right-0 h-0.5"
                    style={{ background: `linear-gradient(90deg, ${match.homeColor}, #ccff00 50%, ${match.awayColor})`, opacity: 0.8 }}
                  />
                )}
                <div className="flex items-center justify-between mb-3">
                  ...
                  {match.status === "live" ? (
                    ...
                    <span className="chip chip-amber"><Clock className="w-2.5 h-2.5" /> {match.time}</span>
                  )}
                </div>
                  ...
                  {([
                    ...
                    <div key={ti} className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="text-[13px] font-semibold truncate" style={{ color: isSel ? "#f5f9f3" : "#9aaa93" }}>{team}</span>
                      </div>
                      {match.status !== "upcoming" && (
                        <span className="text-sm font-mono font-extrabold flex-shrink-0" style={{ color: isSel ? color || "#f5f9f3" : "#9aaa93" }}>{score}</span>
                      )}
                    </div>
                  ))}
                </div>
                <div className="flex items-center justify-between mt-3 pt-2.5 text-[9px] font-mono"
        ...
        {/* ══ Subtle "Check Venues" jump-link ══ */}
        <div className="flex justify-end -mt-1">
          <button
            onClick={() => document.getElementById("venue_list_anchor")?.scrollIntoView({ behavior: "smooth", block: "start" })}
            ...
            style={{ background: "rgba(204,255,0,0.04)", border: "1px solid rgba(204,255,0,0.14)", color: "#c5d0c1", letterSpacing: "0.04em" }}
            onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(204,255,0,0.07)"; e.currentTarget.style.color = "#ccff00"; }}
            onMouseLeave={(e) => { e.currentTarget.style.background = "rgba(204,255,0,0.04)"; e.currentTarget.style.color = "#c5d0c1"; }}
          >
            Check venues
        ...
        {/* ══ Search + Filters ══ */}
        <div className="rounded-2xl p-4 space-y-3" style={{ background: "rgba(10,18,8,0.55)", border: "1px solid rgba(255,255,255,0.05)" }}>
          <div className="relative">
            ...
            <input
              ...
              style={{ background: "rgba(5,9,3,0.8)", border: "1px solid rgba(255,255,255,0.07)", color: "#f5f9f3", fontFamily: "var(--font-sans)" }}
              ...
              return (
                <button key={f} onClick={() => setFilter(f)}
                  ...
                  style={active
                    ...
                    : { background: "rgba(20,32,16,0.7)", color: "#7a8a75", border: "1px solid rgba(255,255,255,0.06)" }
                  }
                >
                  {f}
                </button>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="label-mono">
              {aiRankDone ? "AI-Ranked Venues" : "Verified Venue Ranking"}
            </span>
          ...
          <div className="flex items-center gap-1 text-[9.5px] font-mono" style={{ color: "#7a8a75" }}>
            {aiRankLoading ? (
              <>
                <Brain className="w-3 h-3 animate-pulse" style={{ color: "#ccff00" }} />
                AI Ranking…
              </>
            ) : aiRankDone ? (
              <>
                <Brain className="w-3 h-3" style={{ color: "#ccff00" }} />
                Powered by GMI
              </>
            ) : (
              <>
        ...
        >
          <div
            ...
            style={{ width: 36, height: 36, background: "rgba(204,255,0,0.12)", border: "1px solid rgba(204,255,0,0.3)", color: "#ccff00", letterSpacing: "0.05em" }}
          >
            MC
          </div>
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] mt-0.5 leading-relaxed" style={{ color: "#7a8a75" }}>
              {aiRankLoading
                ? "Analysing venues for your party with GMI Cloud AI…"
                : aiRankDone
                ? "Venues ranked by AI for your group size, vibe &amp; team preference."
        ...
        {loading ? (
          ...
          >
            ...
            <button onClick={() => { setQuery(""); setFilter("All"); }}
              className="mt-5 chip chip-neon cursor-pointer" style={{ padding: "6px 16px", borderRadius: "9999px" }}>
              Reset Search
            </button>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4 pl-5">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          <h4 className="text-[13.5px] font-bold leading-tight truncate" style={{ color: "#f5f9f3" }}>{venue.name}</h4>
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] font-mono" style={{ color: "#7a8a75" }}>
                          ...
                          <span className="px-1.5 py-0.5 rounded-md text-[9.5px]"
                      ...
                      <div className="flex flex-col items-end gap-1 flex-shrink-0">
                        {aiRank ? (
                          <div
                            ...
                            style={{ background: "rgba(204,255,0,0.14)", border: "1px solid rgba(204,255,0,0.35)", color: "#ccff00" }}
                          >
                            <Brain className="w-3 h-3" />
                            {Math.round(aiRank.aiRankScore * 100)}%
                          </div>
                        ) : (
                          <div
                            className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-[12px] font-mono font-bold"
                        ...
                        <div className="w-12 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.07)" }}>
                          <div className="h-full rounded-full"
                            style={{ width: `${dPct}%`, background: `linear-gradient(90deg, ${dc}88, ${dc})` }} />
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Collapsed: AI reason or concierge teaser */}
                    {!isOpen && (
                      <div
                      ...
                      >
                        ...
                        <p className="text-[11px] leading-relaxed line-clamp-1" style={{ color: "#9aaa93" }}>
                          {aiRank?.aiRankReason ?? venue.conciergeInsight}
                        </p>
                      </div>
                    )}
                      ...
                      {isOpen && (
                        ...
                        >
                          <div className="mt-4 pt-4 space-y-4" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                            ...
                            {/* AI rank reason (if available) */}
                            {aiRank && (
                              <div
                                className="rounded-xl p-3.5 flex gap-3"
                                style={{ background: "rgba(5,9,3,0.75)", border: "1px solid rgba(204,255,0,0.22)", boxShadow: "inset 0 1px 0 rgba(255,255,255,0.04)" }}
                              >
                                <div
                                  className="flex-shrink-0 flex items-center justify-center rounded-lg text-[9px] font-mono font-extrabold"
                                  style={{ width: 30, height: 30, background: "rgba(204,255,0,0.1)", border: "1px solid rgba(204,255,0,0.3)", color: "#ccff00" }}
                                >
                                  <Brain className="w-4 h-4" />
                                </div>
                                <div className="flex-1">
                                  <span className="block text-[9.5px] font-mono font-bold uppercase mb-1"
                                    style={{ color: "#ccff00", letterSpacing: "0.15em" }}>
                                    AI Match Score · {Math.round(aiRank.aiRankScore * 100)}%
                                  </span>
                            ...
                            <div
                              className="rounded-xl p-3.5 flex gap-3"
                              style={{ background: "rgba(5,9,3,0.75)", border: "1px solid rgba(204,255,0,0.16)", boxShadow: "inset 0 1px 0 rgba(255,255,255,0.04)" }}
                            >
                              <div
                                className="flex-shrink-0 flex items-center justify-center rounded-lg text-[9px] font-mono font-extrabold"
                            ...
                            <div className="space-y-2.5">
                              <span className="block text-[9.5px] font-mono font-bold uppercase"
                                ...
                                {([12, 35, 65, 100, 82] as const).map((h, i) => {
                                  ...
                                  return (
                                    <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
                                      <div className="w-full rounded-sm density-bar"
                                        style={{
                                          height: `${h}%`,
                                          background: isKO ? `linear-gradient(to top, ${dc}aa, ${dc})` : h > 50 ? "rgba(204,255,0,0.2)" : "rgba(255,255,255,0.07)",
      ...
      {activeRsvpVenue && (
        ...
        >
          <motion.button
            ...
            style={{
              background: "linear-gradient(135deg, rgba(204,255,0,0.2) 0%, rgba(10,18,7,0.95) 100%)",
              border: "1px solid rgba(204,255,0,0.4)",
              boxShadow: "0 14px 32px rgba(0,0,0,0.55), 0 0 24px rgba(204,255,0,0.25), inset 0 1px 0 rgba(255,255,255,0.06)",
          ...
          >
            <span
              ...
              style={{ background: "rgba(204,255,0,0.18)", border: "1px solid rgba(204,255,0,0.42)" }}
            >
              <PhoneCall className="w-3.5 h-3.5" style={{ color: "#ccff00" }} />
              <span className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full"
                style={{ background: "#ccff00", boxShadow: "0 0 8px #ccff00", animation: "live-pulse 1.6s infinite" }} />
            </span>
            <div className="flex flex-col items-start leading-tight min-w-0">
              <span className="text-[10px] font-mono font-bold uppercase" style={{ color: "#ccff00", letterSpacing: "0.16em" }}>RSVP AI</span>
              <span className="text-[9.5px] font-mono truncate max-w-[180px]" style={{ color: "#9aaa93", letterSpacing: "0.04em" }}>
                {activeRsvpVenue.name}
              </span>
            </div>
            <Sparkles className="w-3.5 h-3.5 ml-1 flex-shrink-0 transition-transform group-hover:scale-110" style={{ color: "#ccff00", opacity: 0.85 }} />
          </motion.button>
        </div>
      )}

      {/* ══ Voice-call RSVP modal ══ */}
      <VoiceCallModal
