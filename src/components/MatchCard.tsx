...
export default function MatchCard({ match, onClick, compact }: Props) {
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-gray-800 rounded-2xl border border-gray-700 transition-all ${
        onClick ? "cursor-pointer hover:border-blue-500 active:scale-[0.98]" : ""
      } ${compact ? "p-3" : "p-4"}`}
    >
      {match.status === "live" && (
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col items-center flex-1 min-w-0">
          ...
          <span className="text-sm font-semibold mt-1 truncate max-w-full text-center">
            {match.homeTeam.name}
          </span>
        </div>
