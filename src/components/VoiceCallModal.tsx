...
import type { Venue, Match, FanProfile, CallStatus } from "@/lib/types";
...
export default function VoiceCallModal({ venue, match, profile, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-[560px] mx-auto bg-gray-900 rounded-t-3xl slide-up max-h-[85vh] flex flex-col border border-gray-800 border-b-0">
        <div className="flex items-center justify-between p-4 border-b border-gray-800">
          <div>
            <h2 className="font-bold text-lg">📞 Call &amp; Book</h2>
            <p className="text-sm text-gray-400">
              {venue.name} · Party of {profile.partySize ?? 2}
            </p>
          </div>
          <button
            onClick={handleClose}
          ...
          {status === "idle" && (
            <div className="text-center py-6">
              ...
              <p className="text-gray-400 text-sm mb-2">
                Our AI agent will call {venue.name} and book a table for {match.homeTeam.flag}{" "}
                {match.homeTeam.name} vs {match.awayTeam.flag} {match.awayTeam.name}.
              </p>
              <p className="text-xs text-gray-500">
                Party of {profile.partySize ?? 2} · Demo mode (no real call placed)
              </p>
            </div>
          )}
